...
public class SequenceLocker1 : MonoBehaviour
{
    ...
    IEnumerator CheckSphereStates()
    {
        ...
        while (locker == true) // Utilisation de "==" pour comparer les valeurs
        {
            ...
            if (sphere1IsGreen && sphere2IsGreen && sphere3IsGreen && sphere4IsGreen)
            {
                // Changer les canvas
                Canvas_lock1.SetActive(false);
                Canvas_lock2.SetActive(false);
                Canvas_lock3.SetActive(false);
                Canvas_lock4.SetActive(false);
                yield return new WaitForSeconds(0.5f);
                Canvas.SetActive(true);
                locker = false; 
                GetComponent<SequenceObjectMover1>().enabled = false;
